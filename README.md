...
2.  **Markdown 문자열을 평문으로 변환 (`markdown.js`)**
    ...
    *함수 시그니처:*
    `mdToText(markdown, useKakaoMarkdown = false)`

    *카카오 마크다운 모드 (`useKakaoMarkdown === true`)*
    - 굵게/기울임/굵게+기울임은 특수문자 변환 대신 원문 마크다운을 유지합니다.
    - 인라인 코드는 `` `code` ``, 코드 블록은 장식 박스 내부의 3중 백틱 코드블록으로 출력합니다.
    - 인용문은 연속된 블록 단위로 3중 백틱으로 감싸고, 내부는 `‖` 접두사로 표현합니다.
    - 헤딩은 전체를 `**【...】**` 형태로 출력하며, 헤딩 내부의 볼드는 기존 볼드 특수문자 변환 규칙을 유지합니다.
